...
export async function settleAuction(itemId) {
    try {
        ...
        if (winningBid) {
            ...
            await itemsCollection.updateOne({ _id: item._id }, { $set: {
                    status: 'inactive',
            ...
            // 流標邏輯
            await itemsCollection.updateOne({ _id: item._id }, { $set: { status: 'unsold' } });
        }
    }
    catch (error) {
