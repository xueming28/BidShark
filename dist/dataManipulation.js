...
// import fs from 'fs'; // 移除 fs，因為我們不再操作實體檔案
import { ObjectId } from "mongodb";
import multer from 'multer';
import path from 'path';
// import { fileURLToPath } from 'url'; // 如果不需要 __dirname 也可以移除
import { settleAuction } from './auctionService.js'; // 確保是 .js
import { establishChat } from "./chat.js"; // 確保是 .js
...
// 更新個人資料
dataRouter.post('/updateUserInfo', async (req, res) => {
    try {
        if (!req.session?.user) {
        ...
        if (phoneNumber) {
            await users.updateOne({ _id: new ObjectId(id) }, { $set: { phone: phoneNumber } });
        }
        // 更新 session
...
// 建立新拍賣品
dataRouter.post('/auctions/create', (req, res) => {
...
// 首頁：取得所有進行中的拍賣品
dataRouter.get('/auctions', async (req, res) => {
    try {
        const db = await connectDB();
...
dataRouter.get('/auctions/:id', async (req, res) => {
    try {
        const db = await connectDB();
        ...
        if (item.sellerId && ObjectId.isValid(item.sellerId.toString())) {
            seller = await db.collection('Users').findOne({ _id: new ObjectId(item.sellerId.toString()) }, { projection: { name: 1 } });
        }
        if (item.dSale) {
            res.json({
                success: true,
...
// 出價 
dataRouter.post('/auctions/:id/bid', async (req, res) => {
    if (!req.session?.user?.id) {
        return res.status(401).json({ success: false, message: 'Please log in first' });
    }
    ...
    try {
        ...
        await db.collection('auctionItems').updateOne({ _id: item._id }, {
...
dataRouter.get('/myItems', async (req, res) => {
    try {
        if (!req.session?.user?.id) {
...
dataRouter.get('/auctions/:id/edit', async (req, res) => {
    try {
...
dataRouter.post('/auctions/:id/edit', (req, res) => {
    upload(req, res, async (err) => {
        if (err) {
        ...
        try {
            ...
            if (bidsCount > 0) {
                ...
                if (item.dSale) {
                    ...
                    if (req.body.startPrice) {
                        ...
                        if (isNaN(sp) || sp <= 0)
                            return res.status(400).json({ success: false, message: 'Invalid startPrice' });
            ...
            if (pushOps.images)
                finalUpdate.$push = { images: { $each: pushOps.images } };
...
dataRouter.post('/auctions/:id/buy/:amt', async (req, res) => {
    try {
        if (!req.session?.user?.id) {
            return res.status(401).json({ success: false, message: 'Please log in first' });
        }
        ...
        if (item.stock <= 0) {
            await db.collection('auctionItems').updateOne({ _id: item._id }, { $set: { status: "inactive" } });
            return res.status(400).json({ success: false, message: 'Item is out of stock' });
        }
        const updateResult = await db.collection('auctionItems').updateOne({ _id: item._id }, { $set: { stock: item.stock - amt } });
