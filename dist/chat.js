...
export async function establishChat(userAId, userBId, subject) {
...
chatRouter.get('/getYourChats', async (req, res) => {
...
chatRouter.post('/sendMessage', async (req, res) => {
    ...
    const chatsCollection = db.collection('chat');
    try {
        const result = await chatsCollection.updateOne({ _id: new ObjectId(chatId.trim()) }, {
            $push: {
                chat: {
